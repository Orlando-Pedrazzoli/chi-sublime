...
- **Site público** (`/`, `/servicos`, `/marcacoes`): apresentação do salão e fluxo de marcação online.
...
Construídos em fases anteriores (não revistos nesta iteração): site de reservas (`/marcacoes`), catálogo de serviços (`/servicos`), autenticação (`/entrar`, `/registar`, recuperação de password) e área de cliente (`/conta`). O registo de conta e a recuperação de password disparam emails transacionais.
